...
; @brief:   corrimiento par e impar
; @fecha:   14/01/2023
; @autora:  Salirrosas  Seminario  Shirley  Jasmin
; @Versión  y  programa: MPLAB  X  IDE  v6.00
;------------------------------------------------- ------------------------------------
    PROCESSOR 18F57Q84
#include "CONFIGURACIONBITS.inc"  // config statements should precede project file includes.
#include <xc.inc>  
...
BOTON:
    BANKSEL   LATA
    CLRF      LATC,b
    BCF       LATE,1,b	;enciende el led cuano hay corrimiento par
    BCF       LATE,0,b	;enciende cundo hay corrimiento impar
    BTFSC     PORTA,3,b	;salta si es cero 
    goto      BOTON	;cuendo no presione ejecuata esta instruccion 
    goto      PAR
...
PAR:
    ...
    MOVLW 00000010B ;carga el acomulador con valor 2
    MOVWF LATC,1    ;enciende el led 1
    CALL  Delay_500ms,1	;se presenta un retardo 
    BTFSS PORTA,3,0 ;salta si led no esta encendido
    GOTO  BOTON1   ;ejecuta cuando esta presionado el boton
    
    BSF   LATE,1,b  ;prende el led de corrimiento par 
    BCF   LATE,0,b  ;apaga el led de corrimiento impat
    MOVLW 00001000B ;carga el acomulador con valor de 8
